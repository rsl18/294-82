...
class PascalVOC(data.Dataset):
  ...
  def run_eval(self, results, save_dir, logger=None):
    ...
    from tools.reval import from_dets
    results = from_dets(
        "voc_2007_test"
        , '{}/results.json'.format(save_dir)
        , comp_mode=False
...
def write_pascal_eval_results(results, logger):
    for k, v in results.items():
        if k == "Mean AP":
            ...
            logger.write(f"{k}: AP: {results[k][2]}, recall: {results[k][0]}, precision: {results[k][1]}\n")
