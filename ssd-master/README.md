...
python train.py --config-file configs/vgg_ssd300_xview_coco.yaml
```
### Multi-GPU training
...
export NGPUS=4
python -m torch.distributed.launch --nproc_per_node=$NGPUS train_ssd.py --config-file configs/vgg_ssd300_xview_coco.yaml SOLVER.WARMUP_FACTOR 0.03333 SOLVER.WARMUP_ITERS 1000
```
...
```bash
# Train on COCO trainval35k, and override two default config values,
# MODEL.DEVICE and SOLVER.BATCH_SIZE, setting them to 'cpu' and 4 resp.:
python train_ssd.py --config-file configs/ssd300_xview_coco.yaml --vgg weights/pretrained/vgg16_reducedfc.pth MODEL.DEVICE cpu SOLVER.BATCH_SIZE 4
```
...
python test.py --config-file configs/vgg_ssd300_xview_coco.yaml
```
...
export NGPUS=4
python -m torch.distributed.launch --nproc_per_node=$NGPUS test.py --config-file configs/vgg_ssd300_xview_coco.yaml
```
...
```bash
python demo.py --config-file configs/vgg_ssd300_xview_coco.yaml --images_dir demo --ckpt https://github.com/lufficc/SSD/releases/download/1.2/vgg_ssd300_voc0712.pth
```
