...
from test_net import eval_test
...
def parse_args():
  ...
  """
    ...
    parser.add_argument(
        "--eval_interval",
        dest="eval_interval",
        help="iteration interval between evaluating checkpoints",
        default=1,
        type=int,
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    elif args.dataset == "xview_coco":
        args.imdb_name = "xview_coco_train"
        args.imdbval_name = "xview_coco_val"
        args.set_cfgs = [
            "ANCHOR_SCALES",
            "[4,8,16,32]",
            "ANCHOR_RATIOS",
    ...
    # Setup output dir and stores all input args
    emp = ExperimentManager("faster_rcnn", {"cfg": cfg, "args": vars(args)})
    output_dir = emp.get_output_dir()
    ...
    model_manager = {}
    model_manager["best"] = None
    model_manager["new"] = None
    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        # save_name = os.path.join(output_dir, 'faster_rcnn_{}_{}_{}.pth'.format(args.net, args.dataset, args.session, epoch, step))
        save_name = os.path.join(
            output_dir,
            "{}_{}_{}_{}_{}.pth".format(
                args.net, args.dataset, args.session, epoch, step
            ),
        )
        save_checkpoint(
            {
        ...
        if epoch % args.eval_interval == 0:
            if "coco" in args.dataset:
                new_coco_eval = eval_test(
                    fasterRCNN, args, cfg, test_imdb, test_dataloader, output_dir
                )
                test_info = {
                    "AP IoU=0.50:0.95": new_coco_eval.stats[0],
                    "AP IoU=0.50": new_coco_eval.stats[1],
                    "AP IoU=0.75": new_coco_eval.stats[2],
                    "AR IoU=0.50:0.95": new_coco_eval.stats[8],
                }
                logger.add_scalars(
                    "logs_s_{}/area_all_max_dets".format(args.session),
                    test_info,
                    epoch * iters_per_epoch,
                )
                model_manager["new"] = {
                    "checkpoint_file": save_name,
                    "coco_eval": new_coco_eval,
                }
                if not model_manager["best"] is None:
                    old_coco_eval = model_manager["best"]["coco_eval"]
                    # stats[1] corresponds to mAP at IOU=0.5, maxDets, all areaRng
                    if new_coco_eval.stats[1] > old_coco_eval.stats[1]:
                        print(
                            f"new is better than old \n new stats: {new_coco_eval.stats[1]} old stats: {old_coco_eval.stats[1]}"
                        )
                        os.remove(model_manager["best"]["checkpoint_file"])
                        # rename detection files to #best detection_results.pkl  detections.pkl  detections_val_results.json
                        move(
                            os.path.join(output_dir, "detection_results.pkl"),
                            os.path.join(output_dir, "best_detection_results.pkl"),
                        )
                        move(
                            os.path.join(output_dir, "detections.pkl"),
                            os.path.join(output_dir, "best_detections.pkl"),
                        )
                        move(
                            os.path.join(output_dir, "detections_val_results.json"),
                            os.path.join(
                                output_dir, "best_detections_val_results.json"
                            ),
                        )
                        model_manager["best"] = model_manager["new"]
                    else:
                        os.remove(model_manager["new"]["checkpoint_file"])
                else:
                    model_manager["best"] = model_manager["new"]

    emp.mark_dir_if_complete()
