...
def parse_args():
    ...
    parser.add_argument(
        "--load_name", dest="load_name", help="filename of checkpoint", type=str
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    if args.dataset == "pascal_voc":
    ...
    elif str(args.dataset).startswith("xview_coco"):
        args.imdb_name = f"{args.dataset.strip()}_train"
        args.imdbval_name = f"{args.dataset.strip()}_val"
        args.set_cfgs = ["ANCHOR_SCALES", "[4,8,16,32]", "ANCHOR_RATIOS", "[.5,1,2]"]
        ...
        raise f"dataset: '{args.dataset}' is not supported!"
    ...
    checkpoint_file = os.path.join(args.load_dir, args.load_name)
    ...
    if args.dataset == "xview_coco":
        new_coco_eval = eval_test(fasterRCNN, args, cfg, imdb, dataloader, output_dir)
    else:
        eval_test(fasterRCNN, args, cfg, imdb, dataloader, output_dir)
