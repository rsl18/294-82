...
class xview_coco(imdb):
    def __init__(self, dataset_name: str, image_set: str):
        imdb.__init__(self, dataset_name)
        # COCO specific config options
        self.config = {"use_salt": True, "cleanup": True}
        # name, paths
        self._image_set = image_set
        self._data_path = osp.join(cfg.DATA_DIR, "Xview")
        ...
        self._view_map = {
            "minival": "val",  # 5k val2014 subset
            "valminusminival": "val",  # val2014 \setminus minival2014
            "test-dev": "test",
            "valminuscapval": "val",
            "capval": "val",
            "captest": "val",
    ...
    def _get_ann_file(self):
        return osp.join(
            self._data_path,
            "annotations",
            "xview_coco_v2_" + self._image_set + "_chipped.json",
        )
    ...
    def _do_detection_eval(self, res_file, output_dir):
        ...
        self.coco_eval = COCOeval(self._COCO, coco_dt)
        self.coco_eval.params.useSegm = ann_type == "segm"
        self.coco_eval.evaluate()
        self.coco_eval.accumulate()
        self._print_detection_eval_metrics(self.coco_eval)
        eval_file = osp.join(output_dir, "detection_results.pkl")
        with open(eval_file, "wb") as fid:
            pickle.dump(self.coco_eval, fid, pickle.HIGHEST_PROTOCOL)
